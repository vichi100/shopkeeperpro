...
export default class CustomerListScreen extends Component {
  ...
  async componentDidMount() {
    ...
    if (contacts.total > 0) {
      ...
      var contactDataArrayStorex = await AsyncStorage.getItem(
        "contactDataArrayStore"
      );
      // console.log('contactDataArrayStorex: '+contactDataArrayStorex.length)
      if (
        contactDataArrayStorex === null ||
        contactDataArrayStorex === undefined
      ) {
        console.log("called _sendCustomerDetailsToserver");
        this._sendCustomerDetailsToserver(contactDataArray);
        await AsyncStorage.setItem(
          "contactDataArrayStore",
          JSON.stringify(contactDataArrayStore)
        );
      } else {
        var validList = [];
        ...
        if (validList.length !== 0) {
          ...
          console.log("not called _sendCustomerDetailsToserver");
          var customerContactDetailsDataStr = await AsyncStorage.getItem(
            "customerContactDetailsData"
          );
          if (
            customerContactDetailsDataStr === null ||
            customerContactDetailsDataStr === undefined
          ) {
            console.log("customerContactDetailsData undefined");
            this._sendCustomerDetailsToserver(contactDataArray);
          }
          var customerContactDetailsData = JSON.parse(
            customerContactDetailsDataStr
          );
          this.setState({
            dataSource: customerContactDetailsData,
            arrayholder: customerContactDetailsData,
            isLoading: false
          });
        }
      }
  ...
  _sendCustomerDetailsToserver = async contactsData => {
    ...
    axios({
      ...
      await AsyncStorage.setItem(
        "customerContactDetailsData",
        JSON.stringify(customerContactDetailsData)
      );
    });
  };
