...
openCreditOrderList = () =>{
  ...
  async componentDidMount() {
    ...
    if (contacts.total > 0) {
      ...
      var contactDataArrayStorex = await AsyncStorage.getItem("contactDataArrayStore");
      // console.log('contactDataArrayStorex: '+contactDataArrayStorex.length)
      if(contactDataArrayStorex === null || contactDataArrayStorex === undefined){
        console.log('called _sendCustomerDetailsToserver');
        this._sendCustomerDetailsToserver(contactDataArray)
        await AsyncStorage.setItem("contactDataArrayStore", JSON.stringify(contactDataArrayStore));
      }else{
        var validList = []
